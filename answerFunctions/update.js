...
const updateEmployee = async(init) => {
    try{
        ...
        const dbEmployee = await db.promise().query('SELECT id, first_name, last_name FROM employee');
        ...
        var rolesList = dbRoles[0].map(({title, department_id}) => ({name: title, value: department_id}));
